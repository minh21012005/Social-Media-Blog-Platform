...
export function HomePage({ navigate, mutedUserIds = new Set() }) {
  ...
  const filteredFeatured = featured && !mutedUserIds.has(featured.author?.id) ? featured : null
  const filteredEditorPicks = editorPicks.filter((article) => article.author && !mutedUserIds.has(article.author.id))
  const filteredLatest = latest.filter((article) => article.author && !mutedUserIds.has(article.author.id))
