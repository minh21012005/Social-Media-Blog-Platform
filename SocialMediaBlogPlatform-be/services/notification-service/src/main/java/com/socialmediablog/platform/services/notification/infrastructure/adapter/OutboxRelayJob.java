...
public class OutboxRelayJob {
    ...
    public void relay() {
        ...
        log.debug("[OutboxRelay][notification-service] Processing {} pending event(s)", pending.size());
        ...
        for (JpaOutboxEventEntity event : pending) {
            try {
                ...
                log.debug("[OutboxRelay][notification-service] Published event id={} type={}", event.id(), event.getEventType());
            } catch (Exception ex) {
                log.error("[OutboxRelay][notification-service] Failed to publish event id={} type={}: {}",
