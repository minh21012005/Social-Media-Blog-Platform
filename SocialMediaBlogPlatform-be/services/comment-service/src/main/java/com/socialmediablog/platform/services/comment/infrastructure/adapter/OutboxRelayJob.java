...
public class OutboxRelayJob {
    ...
    public void relay() {
        ...
        log.debug("[OutboxRelay][comment-service] Processing {} pending event(s)", pending.size());
        ...
        for (JpaOutboxEventEntity event : pending) {
            try {
                ...
                log.debug("[OutboxRelay][comment-service] Published event id={} type={}", event.getId(), event.getEventType());
            } catch (Exception ex) {
                log.error("[OutboxRelay][comment-service] Failed to publish event id={} type={}: {}",
