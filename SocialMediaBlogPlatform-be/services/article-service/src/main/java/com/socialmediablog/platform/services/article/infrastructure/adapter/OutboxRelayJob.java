...
public class OutboxRelayJob {
    ...
    public void relay() {
        ...
        log.debug("[OutboxRelay][article-service] Processing {} pending event(s)", pending.size());
        ...
        for (JpaOutboxEventEntity event : pending) {
            try {
                ...
                log.debug("[OutboxRelay][article-service] Published event id={} type={}", event.getId(), event.getEventType());
            } catch (Exception ex) {
                log.error("[OutboxRelay][article-service] Failed to publish event id={} type={}: {}",
