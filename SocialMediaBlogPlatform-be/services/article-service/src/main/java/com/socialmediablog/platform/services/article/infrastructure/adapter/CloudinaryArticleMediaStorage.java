...
import com.socialmediablog.platform.services.article.config.CloudinaryProperties;
...
@Component
public class CloudinaryArticleMediaStorage implements ArticleMediaStorage {

    private final CloudinaryProperties properties;
    private final RestClient restClient;

    public CloudinaryArticleMediaStorage(CloudinaryProperties properties) {
        this.properties = properties;
        this.restClient = RestClient.create();
    }

    @Override
    public StoredArticleMedia upload(String originalFilename, String mimeType, byte[] content) {
        if (!properties.configured()) {
            throw new IllegalStateException("Cloudinary is not configured for article media storage");
        }
        long timestamp = System.currentTimeMillis() / 1000;
        String folder = properties.getArticleFolder();
